...
def test_trailing_e_not_consumed_as_exponent():
    # '5e' is not a valid exponent -> '5' number then 'e' word.
    toks = tokenize_line("5e")
    assert toks[0].type is TokenType.NUMBER and toks[0].value == "5"
    assert toks[1].type is TokenType.WORD and toks[1].value == "e"
...
def test_number_then_word_boundary():
    ...
    assert toks[1].type is TokenType.WORD
    assert toks[1].value == "cm"
