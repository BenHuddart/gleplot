...
def test_headerless_hand_written_dat_gets_default_column_names(tmp_path):
    """A hand-written .gle referencing a headerless .dat (no header row at
    all) still gets column_names populated with the stable defaults on
    parse, so it acquires a header on next save rather than staying
    headerless forever.

    The sidecar name matches gleplot's own '{prefix}_{N}.dat' import
    heuristic (see dataio.classify_data_file) so it is recognized as an
    "import" series (array-backed ax.lines entry) rather than a
    "reference" (file_series entry) -- column_names only applies to the
    former.
    """
